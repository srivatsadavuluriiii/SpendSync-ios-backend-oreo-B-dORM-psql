...
async function createPaymentIntent(req, res, next) {
  try {
    ...
    const updatedSettlement = await settlementService.updateSettlementPayment(
      settlementId,
      {
...
async function processPayment(req, res, next) {
  try {
    ...
    await settlementService.updateSettlementPayment(
      settlementId,
      {
    ...
    res.json({
      ...
      data: {
        ...
        paymentIntentClientSecret: 
          paymentResult.status === 'requires_action' ? paymentResult.client_secret : null
      }
    });
  } catch (error) {
    next(error);
