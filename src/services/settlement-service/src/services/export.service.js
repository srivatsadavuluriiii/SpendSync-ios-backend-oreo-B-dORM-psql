...
function generateSettlementsJson(settlements, format = 'basic') {
  ...
  const formattedSettlements = settlements.map(settlement => {
    ...
    if (format === 'basic') {
      ...
      const { id, createdAt, amount, currency, payerId, receiverId, status, 
              groupId, notes, completedAt } = formattedData;
      return { id, createdAt, amount, currency, payerId, receiverId, status, 
...
async function exportUserData(userId, options) {
  ...
  if (dataType === 'balances' || dataType === 'all') {
    ...
    if (format === 'json') {
      ...
      const balancesByGroup = Object.entries(balances.balancesByGroup).map(([groupId, currencies]) => 
      ...
      const balancesByUser = Object.entries(balances.balancesByUser).map(([otherUserId, currencies]) => 
        Object.entries(currencies).map(([currency, amount]) => ({
          otherUserId,
          currency,
          amount
