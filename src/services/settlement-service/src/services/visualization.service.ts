 ...
 */
function generateNetworkGraph(settlements: any[]): {
  nodes: Array<{ id: string; balance: number }>;
  ...
  for (const settlement of settlements) {
    ...
    const existingLink = links.find(
      link => link.source === from && link.target === to
    );
 ...
 * @returns Formatted data for visualization
 */
function generateSankeyDiagram(settlements: any[]): {
 ...
 */
function generateDebtHeatmap(balances: Record<string, Record<string, number>>): {
  users: string[];
  data: Array<{ from: string; to: string; amount: number }>;
} {
  const users = new Set<string>();
  const data: Array<{ from: string; to: string; amount: number }> = [];
 ...
 */
function generateSettlementBreakdown(debtGraph: any, settlements: any[]): {
  ...
  finalSettlements: Array<{from: string; to: string; amount: number}>;
  stats: {
 ...
 */
function generateSettlementExplanation(debtGraph: any, settlements: any[], algorithm: string): {
  summary: string;
  ...
  stepByStepExplanation.push(
    `The calculation started with ${breakdown.inputDebts.length} original transactions between ${Object.keys(breakdown.userBalances).length} users.`
  );
  ...
  stepByStepExplanation.push(
    `After calculating net balances, we found ${creditorCount} users are owed money and ${debtorCount} users owe money.`
  );
  
  // Step 3: Optimization process
  stepByStepExplanation.push(
    `The ${algorithm} algorithm was applied to find the optimal settlement plan, reducing the number of transactions from ${breakdown.stats.originalTransactionCount} to ${breakdown.stats.optimizedTransactionCount} (${breakdown.stats.reductionPercentage}% reduction).`
  );
  
  // Create transaction summary
  const transactionSummary = settlements.map(s => 
    `${s.from} pays ${s.amount.toFixed(2)} ${s.currency || ''} to ${s.to}`
  );
  
  // Create overall summary
  const summary = 
    `Using the ${algorithm} algorithm, we optimized ${breakdown.inputDebts.length} original transactions into ${settlements.length} settlements, ` +
 ...
 */
function generateSettlementVisualization(debtGraph: any, settlements: any[]): {
...
export {
  ...
  generateDebtHeatmap,
  generateSettlementBreakdown,
  generateSettlementExplanation,
