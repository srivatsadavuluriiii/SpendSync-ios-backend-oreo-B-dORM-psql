...
async function autoCompleteSettlement(settlementId) {
    ...
    const updatedSettlement = await settlementService.updateSettlementStatus(settlementId, 'completed', { completedAt: new Date() });
...
async function sendSettlementNotification(settlementId, payload) {
    ...
    const notificationResults = await Promise.all(recipients.map(userId => notifyUser(userId, notificationType, settlement)));
    return {
        notificationType,
